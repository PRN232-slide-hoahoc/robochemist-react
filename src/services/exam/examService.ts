...
export class ExamService {
  ...
  private static CACHE_NAMES_KEY = 'exam_matrices_names_cache';
  ...
  /**
   * Get cached matrix names from localStorage
   */
  private static getCachedMatrixNames(): { data: MatrixBasic[]; timestamp: number } | null {
    try {
      const cached = localStorage.getItem(this.CACHE_NAMES_KEY);
      if (!cached) return null;
      ...
      // Check if cache is still valid
      if (now - parsed.timestamp < this.CACHE_DURATION) {
        console.log('[ExamService] Using cached matrix names');
        return parsed;
      }
      
      // Cache expired
      localStorage.removeItem(this.CACHE_NAMES_KEY);
      return null;
    } catch (error) {
      console.error('[ExamService] Error reading matrix names cache:', error);
      return null;
    }
  }
  ...
  /**
   * Save matrix names to cache
   */
  private static setCachedMatrixNames(matrices: MatrixBasic[]): void {
    try {
      const cacheData = {
        data: matrices,
        timestamp: Date.now(),
      };
      localStorage.setItem(this.CACHE_NAMES_KEY, JSON.stringify(cacheData));
      console.log('[ExamService] Matrix names cached successfully');
    } catch (error) {
      console.error('[ExamService] Error writing matrix names cache:', error);
    }
  }
  ...
  static async createExamRequest(
    ...
    try {
      ...
      let errorMessage = error?.response?.data?.message || 'Không thể tạo yêu cầu thi';
      
      // Replace backend error message format with Vietnamese-friendly version
      // Pattern: "Không đủ câu hỏi cho Topic {guid}, QuestionType '{type}', Level '{level}'. Yêu cầu {needed} câu, chỉ có {available} câu trong hệ thống"
      const topicPattern = /Không đủ câu hỏi cho Topic ([a-f0-9-]+), QuestionType '(\w+)', Level '(\w+)'\. Yêu cầu (\d+) câu, chỉ có (\d+) câu trong hệ thống/i;
      const match = errorMessage.match(topicPattern);
      
      if (match) {
        const [, topicId, questionType, level, needed, available] = match;
        
        // Try to get topic name from cache or matrices
        const cachedMatrices = localStorage.getItem('exam_matrices_cache');
        let topicName = topicId; // fallback to ID
        
        if (cachedMatrices) {
          try {
            const { data } = JSON.parse(cachedMatrices);
            const allDetails = data.flatMap((m: any) => m.matrixDetails || []);
            const detail = allDetails.find((d: any) => d.topicId === topicId);
            if (detail?.topicName) {
        ...
        const levelVN = levelNames[level] || level;
        
        errorMessage = `Hệ thống không đủ câu hỏi cho chủ đề "${topicName}", mức độ ${levelVN}. Yêu cầu ${needed} câu, chỉ có ${available} câu trong hệ thống.`;
      }
  ...
  /**
   * Download exam questions only (without answers)
   */
  static async downloadExamQuestions(examId: string): Promise<void> {
    try {
      const response = await axiosInstance.get(
        API_ENDPOINTS.EXAM.EXAM_EXPORT_QUESTIONS(examId),
        {
      ...
      let filename = `DeThi_${examId}.docx`;
      ...
      console.error('Error downloading exam questions:', error);
   ...
   */
  static async downloadAnswerKey(examId: string): Promise<void> {
    try {
      const response = await axiosInstance.get(
        API_ENDPOINTS.EXAM.EXAM_EXPORT_ANSWERS(examId),
        {
          responseType: 'blob', // Important for file download
        }
      );
      ...
      // Extract filename from Content-Disposition header or use default
      const contentDisposition = response.headers['content-disposition'];
      let filename = `DapAn_${examId}.docx`;
      ...
      throw new Error(error?.response?.data?.message || 'Không thể tải xuống đáp án');
    }
  }

  /**
   * Download exam file as Word document (full exam with answers)
   */
